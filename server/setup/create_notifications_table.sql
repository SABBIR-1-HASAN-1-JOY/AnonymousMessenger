...
BEGIN
    ...
    IF content_owner_id IS NOT NULL THEN
        PERFORM create_notification(
            ...
            NEW.entity_type,
            NEW.entity_id,
            actor_name || ' commented on your ' || NEW.entity_type || ': "' || COALESCE(entity_title, 'content') || '"'
        );
    END IF;
...
BEGIN
    ...
    IF content_owner_id IS NOT NULL THEN
        PERFORM create_notification(
            ...
            NEW.entity_type,
            NEW.entity_id,
            actor_name || ' ' || vote_action || ' your ' || NEW.entity_type || ': "' || COALESCE(entity_title, 'content') || '"'
        );
    END IF;
...
-- Trigger function for rating notifications (for rate-my-work posts)
CREATE OR REPLACE FUNCTION notify_rating() RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    -- Check if this is a rate-my-work post
    SELECT user_id, LEFT(post_text, 50) || '...', COALESCE(is_rate_enabled, false)
    INTO post_owner_id, post_excerpt, is_rate_enabled
    FROM post WHERE post_id = NEW.userpost_id;
    
    -- Only notify for rate-my-work posts
    IF is_rate_enabled THEN
        -- Get the actor's name
        SELECT username INTO actor_name FROM "user" WHERE user_id = NEW.user_id;
        ...
        IF post_owner_id IS NOT NULL THEN
            PERFORM create_notification(
                ...
                'post',
                NEW.userpost_id,
                actor_name || ' rated your work (' || NEW.rating || ' stars): "' || COALESCE(post_excerpt, 'your post') || '"'
            );
        END IF;
    END IF;
...
-- Trigger for post ratings (assuming there's a rate_my_work table)
-- If the rating is stored in posts table directly, we'll need to modify this
DROP TRIGGER IF EXISTS trigger_rating_notification ON rate_my_work;
CREATE TRIGGER trigger_rating_notification
    AFTER INSERT ON rate_my_work
    FOR EACH ROW
